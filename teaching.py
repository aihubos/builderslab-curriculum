...
def lesson_intro(slug):
    p=PLANS[slug]
    return f'''<section class="learning-contract" id="learning-goals"><p class="eyebrow">BEFORE YOU TRY</p><h2>오늘 내 손으로 할 수 있게 될 일</h2>{ul(p['objectives'])}<p class="notice">{e(p['prep'])}</p><div class="download-row"><a class="button secondary" href="assets/teaching/{slug}-workbook.pdf" download>인쇄용 교재 PDF ↓</a><a class="text-link" href="slides-{slug}.html">수업 슬라이드 ↗</a><a class="text-link" href="#assignment">독립 과제·평가 기준 ↓</a><a class="text-link" href="#mission">마지막 게임 도전 ↓</a></div><details class="concepts"><summary>실습 전에 이해할 개념 세 가지 <span>＋</span></summary>{''.join(f'<article><h3>{e(x["title"])}</h3><p>{e(x["body"])}</p><p class="concept-example">{e(x["example"])}</p><details><summary>{e(x["question"])}</summary><p>{e(x["answer"])}</p></details></article>' for x in p['concepts'])}</details></section>'''
...
def slides_for(c):
    ...
    slides += [
      ...
      dict(label='FINAL QUEST',title=MISSIONS[c['id']]['title'],lines=['실습 뒤 6개 상황 미션을 해결하고 보스를 클리어하세요.','틀린 문제는 해설과 힌트로 다시 도전합니다. 실제 실습 결과도 함께 확인하세요.','https://aihubos.github.io/builderslab-curriculum/mission-'+c['id']+'.html'],notes='마무리 시간에 교재의 마지막 게임 도전 링크를 엽니다. 전원을 기다리며 답을 대신 고르지 않습니다. 학습자가 왜 그 행동이 맞는지 설명하게 하세요. 실습 체크와 게임 클리어를 모두 마쳐야 과정 마무리로 표시됩니다. 게임 기록은 현재 브라우저에만 남으며 강사에게 자동 제출되지 않습니다.'),
...
def render_teaching(root,courses,head,footer):
    ...
    hub+='''<section class="operating-guide"><h2>현장 운영 순서</h2><div class="prep-grid"><article><span>수업 전</span><h3>기기·계정·폴더 확인</h3><p>준비 안내를 전달하고 설치 도움이 필요한 사람을 파악합니다. 강사는 새 연습 폴더에서 샘플 요청을 직접 실행해봅니다. 충전·와이파이·화면 글자 크기도 확인합니다.</p></article><article><span>수업 중</span><h3>설명할 때는 보고, 실습할 때는 손으로</h3><p>한 번에 한 행동만 시연합니다. 각 단계에서 ‘완료 / 도움 필요’를 확인합니다. 도움은 말로 안내 → 위치만 보여주기 → 다시 혼자 하기 순서로 줍니다.</p></article><article><span>수업 후</span><h3>결과를 보고 다음 행동을 정하기</h3><p>결과 파일과 원문을 함께 봅니다. 평가표에 근거를 남기고 미달 항목은 다른 입력으로 재시도합니다. 수강생에게 원본 파일과 다음 복습 행동을 남깁니다. 마지막에는 과정별 게임의 6개 미션을 해결하고, 틀린 이유를 다시 설명하게 합니다.</p></article></div><details><summary>진도가 달라지거나 인터넷이 안 될 때</summary><p>빠른 학습자는 기능 추가 대신 원문 대조와 두 번째 입력 검사를 합니다. 막힌 수강생은 마지막으로 성공한 단계부터 시작합니다. 네트워크·인증 장애는 다운로드한 원문과 모범 확인 기준으로 관찰·오류 찾기를 진행하고 AI 실행은 미완료로 남겨 보충합니다. 설치를 관찰한 것만으로 개인 설치 완료라고 기록하지 않습니다.</p></details><details><summary>개설 전에 강사가 확정할 운영 정보</summary><p>강사 소개와 실제 경력, 일정·장소·정원·수강료, 기기·계정 준비 조건, 보충 시간과 질문 채널, 신청·취소 안내를 확정해 별도 모집 공지에 담습니다. 이 홈페이지는 자료 제공용이며 신청·결제·수강생 정보를 수집하지 않습니다.</p></details></section>'''
    for c in courses:
